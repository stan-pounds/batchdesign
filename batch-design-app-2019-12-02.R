##############################
# If necessary, install the shiny package

pack.list=installed.packages()
if (!is.element("shiny",rownames(pack.list))) install.packages(shiny)
...
ui=fluidPage(
  ...
  sidebarLayout(
                sidebarPanel(                                     # Begin Side Bar Panel
                            ...
                            actionButton("exit.app","Exit App")   # Exit button
                            ),             
